...
later. The next two sentences of the description shows that the entity Seat should
...
uniquely define a seat as one row can have multiple seats and each number will
appear in every row, so, we need both to define/locate a seat. Thus, the primary
key for the entity Seat is the 4-tuple (\key{cinema\_id}, \pkey{room\_num},
